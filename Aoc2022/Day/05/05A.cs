...
namespace Aoc2022.Day._05
{
    public class _05A
    {
        ...
        private static void ProcessMove(string line)
        {
            ...
            {
                MoveSingleCrate(from, to);
            }

            return;
        }

        private static void MoveSingleCrate(int from, int to)
        {
            var cratesDictonaryItem = cratesDictonary[from].LastOrDefault();
            if (cratesDictonaryItem == null) return;

            cratesDictonary[to].Add(cratesDictonaryItem);
            cratesDictonary[from].RemoveAt(cratesDictonary[from].Count-1);
        }
    }
}
